...
class product_images(orm.Model):
    ...
    def create(self, cr, uid, vals, context=None):
        if vals.get('name') and not vals.get('extention'):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('name') and not vals.get('extention'):
            vals['name'], vals['extention'] = os.path.splitext(vals['name'])
        upd_ids = ids[:]
        if vals.get('name') or vals.get('extention'):
            ...
            for image in images:
                ...
                if vals.get('name') and (image.name != vals['name']) \
                    or vals.get('extention') and (image.extention != vals['extention']):
    ...
    _columns = {
        ...
        'extention': fields.char('File Extension'),
